...
*The variable C (colour) has 4 different levels (Lite, Median, Dark, and Very Dark). Thus, by using dummy coding, we will need n-1 variables to represent n levels. Thus, we have 3 variables, 3 slope parameters, here.*
...
*The colour of female horseshoe crab has significant association with the number of satellite, as tested by the likelihood ratio test. The Poisson regression shows the same pattern as what we observed in the boxplot. That is, the darker the colour, the less the number of satellites a female can get.*
